...
const TICKET = 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-08-LOCAL-CAPTURE-SOURCE-READINESS-01';
const SLUG = 'v2-pf-c-late-am-paper-observation-2026-06-08-local-capture-source-readiness-01';
const SUBSTRATE_SHA = 'a5ff6fd70787f3df01f358ec91f61f471463008c';
const STRATEGY_ID = 'regime_shock_reversion_short_v2_utc_16_18_exclusion';
const SESSION_ID = '2026-06-08-rth';
const SOURCE_FILE = 'D:/Quant-futures-app/tools/rithmic_analytics/data/captures/2026-06-08/MNQ_globex.obs01.jsonl';
const QUOTE_FILE = 'D:/Quant-futures-app/tools/rithmic_analytics/data/captures/2026-06-08/MNQ_globex.mbp1.jsonl';
const SESSION_OPEN_NS = BigInt(Date.parse('2026-06-08T13:30:00.000Z')) * 1_000_000n;
const SESSION_CLOSE_NS = BigInt(Date.parse('2026-06-08T20:00:00.000Z')) * 1_000_000n;
...
const PRIOR_CLOSE_DATE = '2026-06-05';
...
async function loadScopedRegimeLabel(): Promise<JsonRecord> {
  ...
  const pageFallbackRows = [
    ...
    { date: '2026-06-05', VIXCLS: 21.51, VXNCLS: 30.47 },
  ];
  const rowMap = new Map<string, { date: string; VIXCLS: number | null; VXNCLS: number | null }>();
...
function appendBacklogRow(): void {
  const row = [
    ...
    'P1',
    '1.0',
    'V2-PF-C-LATE-AM-PAPER-OBSERVATION-WAIT-FOR-NEXT-FULL-LOCAL-CAPTURE-DAY-03',
    'Assess 2026-06-08 local continuous Rithmic capture source readiness for full-session source-backed feature snapshots without Databento paper runtime or observation-day authority',
...
async function main(): Promise<void> {
  ...
  const recommendedNextTicket =
    determination === DETERMINATION_READY
      ? 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-08-FEATURE-SNAPSHOT-BUILDER-IMPL-01'
      : determination === DETERMINATION_NO_CANDIDATE
        ? 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-08-NO-CANDIDATE-ELIGIBLE-DISPOSITION-01'
  ...
  const final = {
    ...
    script_lf_sha256: sha256FileLf(path.join(ROOT, 'scripts', 'paper', 'run-v2-pf-c-late-am-2026-06-08-local-capture-source-readiness.ts')),
    recommended_next_ticket: recommendedNextTicket,
  };
