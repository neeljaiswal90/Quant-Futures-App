...
const ticket = 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-WAIT-FOR-NEXT-FULL-LOCAL-CAPTURE-DAY-02';
const slug = 'v2-pf-c-late-am-paper-observation-wait-for-next-full-local-capture-day-02';
const priorReportPath = 'artifacts/paper-observation/v2-pf-c-late-am-paper-observation-wait-for-next-full-local-capture-day-01/wait-for-next-full-local-capture-day-report.json';
...
const waitStatus: JsonRecord = { determination, prior_wait_report_path: priorReportPath, prior_progress_minimum: '1/45', prior_progress_preferred: '1/60', full_post_prior_capture_day_available: full.length > 0, latest_post_prior_candidate: latest, all_post_prior_candidates: postPrior, resume_ticket_if_ready: 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-NEXT-DAY-FEATURE-SNAPSHOT-BUILDER-IMPL-01', next_wait_ticket_if_blocked: 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-WAIT-FOR-NEXT-FULL-LOCAL-CAPTURE-DAY-03', interpretation: full.length > 0 ? 'A post-2026-06-04 full RTH local capture day is available.' : 'No post-2026-06-04 full RTH local capture day is available yet; 2026-06-08 remains partial at scan time.' };
...
const report: Record<string, Json> = { ticket, substrate_sha: '0d6a63bda86d7ab7ea1c2281e5bc9e2d83d84091', determination, wait_status: waitStatus, scans, authority_caveat: authorityCaveat };
...
const memo = `# ${ticket} memo\n\n## Result\n\n\`${determination}\`\n\nThe post-2026-06-04 local capture day remains incomplete. The count remains 1/45 and 1/60.\n\n## Latest candidate\n\n${latest ? `- session_date: ${latest.session_date}\n- rth_trade_slots_present: ${latest.rth_trade_slots_present}\n- rth_trade_slots_missing: ${latest.rth_trade_slots_missing}\n- source_file: ${latest.source_file}\n` : '- none found\n'}\n## Boundary\n\nNo runtime, feature snapshot, strategy markers, observation-day increment, or broker/live/Phase 6/roster authority is created.\n`;
writeText(memoPath, memo);
const backlogRow = `${ticket},P1,1.0,V2-PF-C-LATE-AM-PAPER-OBSERVATION-WAIT-FOR-NEXT-FULL-LOCAL-CAPTURE-DAY-01,Refresh wait state for the next post-2026-06-04 full local RTH capture day before additional paper-observation accrual without runtime markers or authority,new_cycle4_v2_research_substrate`;
