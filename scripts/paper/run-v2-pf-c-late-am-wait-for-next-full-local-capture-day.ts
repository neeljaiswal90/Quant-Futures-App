...
const ticket = 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-WAIT-FOR-NEXT-FULL-LOCAL-CAPTURE-DAY-01';
const slug = 'v2-pf-c-late-am-paper-observation-wait-for-next-full-local-capture-day-01';
const priorReportPath = 'artifacts/paper-observation/v2-pf-c-late-am-paper-observation-next-local-capture-day-source-readiness-01/next-local-capture-day-source-readiness-report.json';
...
const sourceFiles = [
  ...
  'D:/Quant-futures-app/tools/rithmic_analytics/data/captures/2026-06-08/MNQ_globex.obs01.jsonl',
].filter(existsSync);
...
const postPrior = allCoverage.filter((entry) => String(entry.session_date) > '2026-06-04');
const full = postPrior.filter((entry) => Number(entry.rth_trade_slots_present) === 390);
const latest = postPrior.sort((a, b) => String(a.session_date).localeCompare(String(b.session_date)) || String(a.source_file).localeCompare(String(b.source_file))).at(-1) ?? null;
...
const waitStatus: JsonRecord = { determination, prior_progress_minimum: '1/45', prior_progress_preferred: '1/60', full_post_prior_capture_day_available: full.length > 0, latest_post_prior_candidate: latest, all_post_prior_candidates: postPrior, resume_ticket_if_ready: 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-NEXT-DAY-FEATURE-SNAPSHOT-BUILDER-IMPL-01', next_wait_ticket_if_blocked: 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-WAIT-FOR-NEXT-FULL-LOCAL-CAPTURE-DAY-02', interpretation: full.length > 0 ? 'A post-2026-06-04 full RTH local capture day is available.' : 'No post-2026-06-04 full RTH local capture day is available yet; continue waiting or rerun after capture completes.' };
...
const report: Record<string, Json> = { ticket, substrate_sha: '78a7a36af6eec6d86d6eccfe69348b2225373490', determination, wait_status: waitStatus, scans, authority_caveat: authorityCaveat };
...
console.log(JSON.stringify({ state: 'PENDING_REVIEW', ticket, substrate_sha: finalReport.substrate_sha, determination, progress: '1/45 and 1/60', full_post_prior_capture_day_available: full.length > 0, latest_post_prior_candidate: latest, observation_day_increment: 0, recommended_next_ticket: full.length > 0 ? waitStatus.resume_ticket_if_ready : waitStatus.next_wait_ticket_if_blocked, output_hashes: finalReport.output_hashes }, null, 2));
