...
const TICKET = 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-04-PAPER-RUNTIME-OBSERVATION-DAY-SCOPE-01';
const SLUG = 'v2-pf-c-late-am-paper-observation-2026-06-04-paper-runtime-observation-day-scope-01';
const SUBSTRATE_SHA = '05a2ca8b43f218927ec2a86a8a301f9f2f16fc72';
const DETERMINATION = 'OBSERVATION_DAY_SCOPE_BLOCKED_REQUIRES_FULL_SESSION_RUNTIME';
const NEXT_TICKET = 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-04-FULL-SESSION-RUNTIME-SCOPE-01';
...
const SOURCE_REPORTS = [
  ...
  { pr: 324, label: 'paper_runtime_candidate_smoke_scope', path: 'artifacts/paper-observation/v2-pf-c-late-am-paper-observation-2026-06-04-paper-runtime-candidate-smoke-scope-01/paper-runtime-candidate-smoke-scope-report.json' },
...
function appendBacklogRow(): void {
  const row = [TICKET, 'P1', '1.0', 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-04-PAPER-RUNTIME-CANDIDATE-SMOKE-IMPL-01', 'Scope whether bounded 2026-06-04 paper-runtime candidate smoke evidence can count as an observation day and define the minimum full-session evidence contract without order broker live Phase 6 roster or observation-credit authority', 'new_cycle4_v2_research_substrate'].join(',');
...
function main(): void {
  ...
  const sourceAnchors: Json[] = SOURCE_REPORTS.map((source) => {
    ...
    if (!existsSync(absolutePath)) throw new Error(`Missing source report: ${source.path}`);
    return { pr: source.pr, label: source.label, path: source.path, lf_sha256: sha256FileLf(absolutePath), size_bytes: statSync(absolutePath).size };
  });
  const pr325 = parseJsonFile(path.join(ROOT, SOURCE_REPORTS[3].path));
  const stratEvalCount = numberField(pr325, 'STRAT_EVAL_count', 'STRAT_EVAL');
  const candidateCount = numberField(pr325, 'CANDIDATE_count', 'CANDIDATE');
  const orderIntentCount = numberField(pr325, 'ORDER_INTENT_count', 'ORDER_INTENT');
  if (stratEvalCount !== 1 || candidateCount !== 1 || orderIntentCount !== 0) throw new Error(`Unexpected PR #325 counts: STRAT_EVAL=${stratEvalCount} CANDIDATE=${candidateCount} ORDER_INTENT=${orderIntentCount}`);
  ...
  const boundedRecords: Json[] = [
    ...
    { record_type: 'OBSERVATION_DAY_SCOPE_DECISION', ticket: TICKET, determination: DETERMINATION, candidate_only_evidence: candidateOnlyEvidence, evidence_contract: evidenceContract, authority_locks: authorityLocks },
  ];
  ...
  const report: JsonRecord = {
    ...
    decision_summary: 'The bounded 2026-06-04 candidate smoke proves source-backed paper-runtime candidate plumbing, but it is a single-snapshot control proof and does not satisfy an observation-day unit. Observation-day increment remains 0 until a full-session/window paper-runtime accounting artifact is scoped and produced.',
  ...
  appendBacklogRow();
  console.log(JSON.stringify({ state: 'PENDING_REVIEW', determination: DETERMINATION, observation_day_eligible: false, observation_day_increment: 0, candidate_only_smoke_sufficient_for_day_credit: false, bounded_jsonl_lf_sha256: boundedSha, report_json_lf_sha256: reportSha, report_md_lf_sha256: reportMdSha, memo_lf_sha256: sha256FileLf(MEMO_MD), recommended_next_ticket: NEXT_TICKET }, null, 2));
}
main();
