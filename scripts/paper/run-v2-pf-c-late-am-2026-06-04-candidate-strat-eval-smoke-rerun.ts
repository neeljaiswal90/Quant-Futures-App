...
const TICKET = 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-04-CANDIDATE-STRAT-EVAL-SMOKE-RERUN-01';
const SLUG = 'v2-pf-c-late-am-paper-observation-2026-06-04-candidate-strat-eval-smoke-rerun-01';
...
const BOUNDED_JSONL = path.join(OUT_DIR, 'bounded-candidate-strat-eval-smoke-rerun.jsonl');
const REPORT_JSON = path.join(OUT_DIR, 'candidate-strat-eval-smoke-rerun-report.json');
const REPORT_MD = path.join(OUT_DIR, 'candidate-strat-eval-smoke-rerun-report.md');
...
const SOURCE_JSONL = path.join(
  ...
  'bounded-feature-snapshot-compat-repair.jsonl',
);
const SOURCE_REPORT_JSON = path.join(
  ...
  'paper-observation',
  'v2-pf-c-late-am-paper-observation-2026-06-04-feature-snapshot-builder-compat-repair-01',
  'feature-snapshot-builder-compat-repair-report.json',
);
...
function loadFeatureSnapshot(): StrategyFeatureSnapshot {
  if (!existsSync(SOURCE_JSONL)) throw new Error(`Missing repaired source snapshot JSONL: ${SOURCE_JSONL}`);
  for (const line of readText(SOURCE_JSONL).split(/\r?\n/)) {
    ...
    if (record.feature_snapshot_id !== FEATURE_SNAPSHOT_ID) throw new Error(`Feature snapshot id mismatch: ${record.feature_snapshot_id}`);
    return reviveTimestampNs(record) as StrategyFeatureSnapshot;
  }
  throw new Error('StrategyFeatureSnapshot record not found in repaired JSONL');
}

function appendBacklogRow(): void {
  const row =
    'V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-04-CANDIDATE-STRAT-EVAL-SMOKE-RERUN-01,P1,1.0,V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-04-FEATURE-SNAPSHOT-BUILDER-COMPAT-REPAIR-01,Rerun the 2026-06-04 candidate strat-eval smoke against the instrument-compatible source-backed snapshot while ORDER_INTENT and all execution observation-day authority remain suppressed,new_cycle4_v2_research_substrate';
...
This rerun loads the PR #322 instrument-compatible source-backed snapshot and runs only the narrow strategy generator for \`${STRATEGY_ID}\`. It proves \`STRAT_EVAL = 1\` and \`CANDIDATE = 1\` for the non-excluded 14:58Z point while keeping \`ORDER_INTENT = 0\`.
...
function main(): void {
  ...
  const result = generateRegimeShockReversionShortV2Utc1618Exclusion({ strategy_id: STRATEGY_ID, snapshot });
  ...
  const determination =
    stratEvalCount === 1 && candidateCount === 1 && orderIntentCount === 0
      ? 'CANDIDATE_STRAT_EVAL_SMOKE_RERUN_PASSED_CANDIDATE_EMITTED_ORDER_INTENT_SUPPRESSED'
      : 'CANDIDATE_STRAT_EVAL_SMOKE_RERUN_INCONCLUSIVE';
  ...
  const candidateSummary: JsonRecord = result.candidate
    ...
    : { candidate_id: null, confidence: null, direction: null, entry_price: null, first_reason: null, risk_points: null, stop_price: null };
  ...
  const records: Json[] = [
    {
      ...
      source_anchor: {
        ...
        source_bounded_feature_snapshot_lf_sha256: sourceHashes.bounded_feature_snapshot_compat_repair_jsonl,
        source_merge_commit: 'c4c009edd08a3bf099732bfaa588442a5d0f769a',
    ...
    { authority_locks: authorityLocks, record_type: 'AUTHORITY_AND_OBSERVATION_GUARDRAILS', ticket: TICKET },
    { determination, record_type: 'DETERMINATION', ticket: TICKET },
  ];
  ...
  const report: JsonRecord = {
    ...
    observation_day_increment: 0,
    source_anchor: {
      feature_snapshot_compat_repair_merge_commit: 'c4c009edd08a3bf099732bfaa588442a5d0f769a',
      source_bounded_feature_snapshot_lf_sha256: sourceHashes.bounded_feature_snapshot_compat_repair_jsonl,
      source_report_lf_sha256: sha256FileLf(SOURCE_REPORT_JSON),
    },
    recommended_next_ticket: 'V2-PF-C-LATE-AM-PAPER-OBSERVATION-2026-06-04-PAPER-RUNTIME-CANDIDATE-SMOKE-SCOPE-01',
    output_hashes: { bounded_candidate_strat_eval_smoke_rerun_jsonl: sha256FileLf(BOUNDED_JSONL) },
  };
  writeFileSync(REPORT_JSON, stableJson(report), 'utf8');
  report.output_hashes = {
    bounded_candidate_strat_eval_smoke_rerun_jsonl: sha256FileLf(BOUNDED_JSONL),
    candidate_strat_eval_smoke_rerun_report_json: sha256FileLf(REPORT_JSON),
  };
  writeFileSync(REPORT_JSON, stableJson(report), 'utf8');
  ...
  report.output_hashes = {
    bounded_candidate_strat_eval_smoke_rerun_jsonl: sha256FileLf(BOUNDED_JSONL),
    candidate_strat_eval_smoke_rerun_report_json: sha256FileLf(REPORT_JSON),
    candidate_strat_eval_smoke_rerun_report_md: sha256FileLf(REPORT_MD),
  };
  writeFileSync(REPORT_JSON, stableJson(report), 'utf8');
  writeFileSync(REPORT_MD, buildReportMarkdown(report), 'utf8');
  ...
  const final = {
    ...
    script_lf_sha256: sha256FileLf(path.join(ROOT, 'scripts', 'paper', 'run-v2-pf-c-late-am-2026-06-04-candidate-strat-eval-smoke-rerun.ts')),
