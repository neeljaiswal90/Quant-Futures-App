...
const SELECTED_SESSIONS = [
  { month: 'feb', regime: 'baseline', sessionId: '2026-02-25-rth' },
  { month: 'mar', regime: 'stress', sessionId: '2026-03-17-rth' },
] as const;
...
async function collectMbp1Records(path: string): Promise<DbnMbp1Record[]> {
  const records: DbnMbp1Record[] = [];
  for await (const record of loadDbnFile(path, 'mbp-1')) {
    records.push(record as DbnMbp1Record);
  }
  return records;
}
...
async function runSession(
  ...
  if (mboFile === undefined || mbp1File === undefined) {
    throw new Error(`Session missing mbo or mbp-1 schema: ${selected.sessionId}`);
  }

  console.log(`[qfa-402] ${selected.sessionId}: loading mbp-1 ${mbp1File.path}`);
  const mbpLoadStart = performance.now();
  const mbp1Records = await collectMbp1Records(mbp1File.path);
  timings.set('load_mbp1_ms', elapsedMs(mbpLoadStart));

  console.log(`[qfa-402] ${selected.sessionId}: generating probes from ${mbp1Records.length} mbp-1 records`);
  const probeStart = performance.now();
  const probes = generateQueueFidelityProbes(mbp1Records, { raw_symbol: session.symbol });
  timings.set('generate_probes_ms', elapsedMs(probeStart));

  console.log(`[qfa-402] ${selected.sessionId}: synthesizing mbp_proxy estimates for ${probes.length} probes`);
  const synthStart = performance.now();
  const synthesized = await computeSynthesizedResults(probes, mbp1Records, session.symbol);
  timings.set('synthesize_mbp_proxy_ms', elapsedMs(synthStart));

  console.log(`[qfa-402] ${selected.sessionId}: replaying mbo reference ${mboFile.path}`);
  ...
  console.log(`[qfa-402] ${selected.sessionId}: ${JSON.stringify(result)}`);
  return result;
}
...
async function main(): Promise<void> {
  ...
  console.log(`[qfa-402] archive_root=${ARCHIVE_ROOT}`);
  ...
  console.log(`[qfa-402] manifest_checks=${JSON.stringify(manifestChecks)}`);
  ...
  const output = {
    ...
    policy: {
      ...
      synthesized_mode: DEFAULT_QUEUE_FIDELITY_POLICY_V1.synthesized_mode,
    },
  ...
  console.log(`[qfa-402] final=${JSON.stringify(output, null, 2)}`);
}
