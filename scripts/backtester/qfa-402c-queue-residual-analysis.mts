...
const SESSION_CONFIGS: readonly SessionConfig[] = Object.freeze([
  ...
  {
    label: 'mar02_first_1800s',
    regime: 'mar_clean_prefix',
    month: 'mar',
    session_id: '2026-03-02-rth',
  ...
  for await (const record of loadDbnFile(mbp1Path, 'mbp-1')) {
    ...
    if (probeWindowEnd !== null && record.ts_event >= probeWindowEnd) {
      while (nextSample < probeWindowEnd) {
        emitSample(nextSample, rawSymbol, topOfBookByInstrument, probes, pendingContexts);
...
async function runSession(config: SessionConfig, manifest: CorpusManifest): Promise<object> {
  ...
  console.log(`[qfa-402c] ${config.label}: generating probes`);
  ...
  const synthWindowEnd = config.scope === 'first_1800s_prefix'
    ? generated.first_sample_ts_ns + config.prefix_duration_ns! + DEFAULT_QUEUE_FIDELITY_POLICY_V1.fill_horizon_ns
    : null;

  console.log(`[qfa-402c] ${config.label}: synthesized mbp_trades_proxy for ${generated.probes.length} probes`);
  const synthStart = nowMs();
  const synthesized = await computeSynthesizedResults(generated.probes, mbp1Path, tradesPath, synthWindowStart, synthWindowEnd, session.symbol);
  stageTimings.synthesize_ms = durationMs(synthStart);
  peakRssMb = Math.max(peakRssMb, rssMb());

  console.log(`[qfa-402c] ${config.label}: MBO reference replay`);
  ...
  console.log(`[qfa-402c] ${config.label}: compare and analyze`);
  ...
  const compared = synthesized.results.map((synthResult) => {
    ...
    return compareQueueFidelityProbe(refResult, synthResult, DEFAULT_QUEUE_FIDELITY_POLICY_V1);
  });
  const summary = summarizeQueueFidelityRegime(config.regime, compared, DEFAULT_QUEUE_FIDELITY_POLICY_V1);
...
async function main(): Promise<void> {
  ...
  const result = Object.freeze({
    ...
    repo_head_expected: '3989c3c0e67615d4fe66d54e3084d472db979ef7',
    archive_root: ARCHIVE_ROOT,
    manifest_hashes: LOCKED_MANIFEST_HASHES,
    policy: Object.freeze({
      mode: 'mbp_trades_proxy',
      sample_interval: DEFAULT_QUEUE_FIDELITY_POLICY_V1.sample_interval,
      fill_horizon_ns: DEFAULT_QUEUE_FIDELITY_POLICY_V1.fill_horizon_ns.toString(),
      depletion_lookback_ns: DEFAULT_QUEUE_FIDELITY_POLICY_V1.depletion_lookback_ns.toString(),
