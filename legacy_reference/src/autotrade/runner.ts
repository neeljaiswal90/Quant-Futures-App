...
async function runLegacySingleInstrumentRunner(options: LegacyRunnerOptions = {}): Promise<void> {
  ...
  //
  // PH0-02A / PH10-01 â€” instantiate a shared ProvenanceProvider so
  ...
  // providers swap (Bookmap â†’ TV fallback, config reload, ML model
  // promote/demote).
  ...
  // PH0-02A â€” now that the real config is loaded, refresh the hash on
  ...
  // PH0-01B — shared freshness FSM, observed by the hard-risk quote
  ...
  freshnessMonitor.onTransition((t) => {
    console.log(
      `[FRESHNESS] ${t.from} → ${t.to} (${t.reason}) source=${t.quote_source} age=${t.age_ms}ms`,
    );
    logWriter.writeFreshnessTransition(t);
  });

  // SAFE-002 / PH10-04 — daily-loss kill switch. Fed by every trade
  ...
  // DATA-08: resolve the fallback-policy phase (v3.1 §1.5) and log it.
  ...
  if (marketDataSelection.startup_action === 'use_bookmap') {
    ...
    // PH0-02A â€” record active providers in provenance.
    ...
    // downgrades to TradingView on first healthy→non-healthy transition.
      ...
      marketDataWatchdog = startMarketDataWatchdog({
        ...
        onDowngrade: (event) => {
          ...
          try {
            ...
            // PH0-02A â€” the fallback path is now active; update
        ...
        onRecovery: (event) => {
          console.log(
            `[LOB-RECOVERY] state=${event.previousState}→${event.currentState} observed — ` +
    ...
    // PH0-02A â€” seed provenance so flat-state candidate logs emitted
  ...
  // PH0-02A â€” seed ml_model_version from the active config. Isotonic
  ...
  if (requireStrictArtifacts) {
    ...
    // MUST match the manifest. Existence alone is insufficient — the
  ...
  const onAnalysis = async (cycleNumber: number): Promise<void> => {
    ...
    // PH0-01B — fail-closed freshness gate on new entries. The FSM is
    ...
    // is forbidden per SOLO_PLAN_V3 §1.1.
    ...
    // SAFE-002 / PH10-04 — circuit breaker entry gate. Fail-closed
    // per SOLO_PLAN_V3 §1.1: a tripped breaker rejects every new
    ...
    if (tradeAllowed && !cooldownBlock && !extensionVetoed && !positionManager.hasOpenPosition() && bestSetup) {
      ...
      if (
        ...
        // sidecar OFI is the source of truth (v3.1 §3.3), its
        // all-or-nothing 10s+30s readiness gate must drive
        // `orderflow_warmup_incomplete` — otherwise a sidecar-ready
        ...
        // → local buffer. The local buffer stays in place so
  ...
  // â”€â”€â”€ V2 Multi-Lane Engine (canonical) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  {
    ...
    const onHardRisk = async (_cycle: number): Promise<void> => {
      ...
      if (quoteResult) {
        ...
        // D-006 — fail-closed on missing input. Silently preserving
      ...
      // SAFE-002 / PH10-04 (D-010) — advance the breaker's session-day
      ...
      if (quoteResult && !quoteService.isStale(quoteResult)) {
        ...
        // PH0-02A â€” keep provenance.quote_source in sync with what
      ...
      if (resolvedMlPolicy.inference_enabled && positionManager.hasOpenPosition()) {
        ...
        if (sinceLastMl >= mlInterval || mlForceEvents) {
          try {
            ...
            // PH0-02A â€” keep provenance.ml_model_version in sync with
