...
- MBP10 price-state parity is accepted for V1 under
  ...
  triangulation.
- MBO provider-internal parity is accepted for V1 under ADR-0002 after DATA-PARITY-10/11
  taxonomy review and INFRA-01F policy.
- Full L2/L3 feature parity, MBP10 size/order-count hard-gate parity, and cross-feed
  order-by-order byte identity are not validated.
...
may proceed for the MBP10 price-state sub-scope and MBO provider-internal sub-scope
accepted by ADR-0002. Do not start SIM-02/SIM-03, generate ML datasets, or advance REL
gates until the corresponding implementation and provider-internal replay evidence exists.
...
Still blocked after INFRA-01E, before the later INFRA-01F MBO decision:
...
INFRA-01F supersedes the MBO-specific policy block by accepting the provider-internal MBO
sub-scope while preserving implementation, SIM, and REL evidence blocks.

DATA-01B-PS consumes this narrow policy decision by publishing MBP10 price-state
`MICROSTRUCTURE` events from rich Rithmic probe/sidecar rows:
...
The report must keep `mbp10_price_state_status = accepted_subscope`,
`size_order_count_status = diagnostic_only`, `mbo_status = accepted_subscope`, and
...
## INFRA-01F Accepted MBO Provider-Internal Policy

INFRA-01F accepts the DATA-PARITY-10/11 MBO evidence as the V1 MBO provider-internal
sub-scope decision:

| Evidence | Result | Policy reading |
| --- | ---: | --- |
| Rithmic MBO timestamp/order-id/price/sequence basics | 100% clean | Rithmic extraction is trusted |
| Databento MBO timestamp/order-id/price/sequence basics | 100% clean | Databento extraction is trusted |
| Strict cross-source signature match | 89.951959% | Action taxonomy differs |
| Structural book-action signature match | ~95.17% | Accepted under documented variance |

Accepted for V1:

- MBO provider-internal order lifecycle tracking.
- Single-provider queue-position estimation.
- Single-provider MBO-derived microstructure features.

Diagnostic only:

- Databento `trade`/`unknown` equivalence to Rithmic streams.
- Cross-source order ID byte identity.
- Rithmic-vs-Databento order-by-order replay parity.
...
- SIM-02/SIM-03, ML/research dataset generation, and REL gates until implementation and
  provider-internal replay evidence exists.

The revised summary report generated by `npm run infra:01f:decision` records
`data01b_mbo_subscope_eligible = true`, `data01_full_eligible = false`, and
`route_to = DATA-01B_MBO_PROVIDER_INTERNAL_SUBSCOPE`.

MBO parity analyzer outputs remain `analysis_only`. The policy report is the reviewed
decision artifact.
