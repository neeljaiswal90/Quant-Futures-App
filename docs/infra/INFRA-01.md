...
Full DATA-01B remains partially blocked because MBO parity is not complete. MBO-derived
features, queue-position features, and order-level features remain blocked.
...
MBO rows remain skipped with `mbo_parity_blocked_action_taxonomy_unresolved` until the
DATA-PARITY-11 action-taxonomy evidence has a reviewed policy decision.
...
price-state sub-scope; full `DATA-01B` remains partially blocked pending MBO parity.
...
Full DATA-01 closure still requires Databento `mbo` availability, normalized Databento MBO
exports, matching rich Rithmic MBO payloads, MBO event/action parity, and a passing revised
INFRA-01 verification report that explicitly routes to `DATA-01`.
