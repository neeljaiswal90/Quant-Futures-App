...
See [ADR-0002](../adr/ADR-0002-cross-source-market-data-parity.md): cross-source
MBP10 price-state disagreement is classified as provider/rendering variance for V1 when
both providers are internally consistent above the reviewed threshold.
ADR-0002 also accepts the MBO provider-internal sub-scope after DATA-PARITY-10/11 showed
clean per-provider MBO extraction and documented cross-source action-taxonomy variance.
...
## Post04D INFRA-01E MBP10 Price-State Decision

INFRA-01E records the final post04D MBP10 price-state decision:

- Rithmic `MBP10` vs Rithmic `L1_QUOTE`: 99.647858% both-side top price within one
  MNQ tick.
- Databento `mbp-10` vs Databento `mbp-1`: 99.965621%.
- Rithmic `L1_QUOTE` vs Databento `mbp-1`: 96.92945%.
- Rithmic `MBP10` vs Databento `mbp-10`: 95.993782%.

Decision:

- `data01b_mbp10_price_state_eligible = true` for the MBP10 price-state sub-scope.
...
- MBP10 price-state reconstruction.
- Top-of-book price-state comparison under documented cross-source tolerance.
- Exchange-event-time alignment.

Not accepted as hard parity evidence:

- MBP10 size parity.
- MBP10 order-count parity.
- Rithmic-vs-Databento byte-identical replay.
- MBO order/action parity before INFRA-01F.

Size and order-count parity remain diagnostic only until separately normalized and
reviewed. Replay byte-identity is provider-internal: Rithmic replay output must be compared
against Rithmic-derived output, and Databento replay output must be compared against
Databento-derived output. REL gates must report cross-source disagreement as telemetry.

Full DATA-01B remained partially blocked at INFRA-01E because MBO policy had not yet been
accepted. INFRA-01F supersedes that MBO-specific block while preserving the implementation
and REL evidence blocks.

DATA-01B-PS is the implementation slice for this accepted sub-scope. It emits
OBS-01-compatible `MICROSTRUCTURE` events for MBP10 price-state snapshots only. Its reports
must preserve:
...
MBO rows remain skipped by DATA-01B-PS because that path is MBP10 price-state-only. After
INFRA-01F, the skip reason is `mbo_accepted_subscope_not_consumed_by_price_state_path`,
which means MBO is accepted as its own provider-internal sub-scope but not consumed by the
price-state publisher.

DATA-01B-MBO is the implementation slice for the accepted MBO provider-internal sub-scope.
It emits source-like OBS-01-compatible `MICROSTRUCTURE` events for Rithmic MBO order
lifecycle updates only. Its reports must preserve:
...
The safe summary report is generated by:

```powershell
npm run infra:01e:decision -- --out reports/infra/infra01_revised_timestamp_gate_post04d_summary.json
```

## Post04D INFRA-01F MBO Provider-Internal Decision

INFRA-01F records the final post04D MBO policy decision:
...
- Timestamp coverage, order ID coverage, price tick alignment, and sequence monotonicity:
  clean for both providers.
- Strict signature match: 89.951959% of Databento events.
- Structural book-action match: approximately 95.17%.
- Material unmatched Databento events come from `trade`/`unknown` action taxonomy variance.

Decision:

- `data01b_mbo_subscope_eligible = true` for provider-internal MBO lifecycle and queue
  work.
...
- Treating Databento `trade`/`unknown` actions as hard failures without a separate mapping.

Full DATA-01B still requires DATA-02-MBO book state, DATA-03 authority FSM, DATA-04 feature
engine, SIM-02/SIM-03 calibration, and provider-internal replay evidence.

REL gates remain blocked until those implementation gates produce their own evidence.
...
- MBP10 price-state parity is accepted for V1 under ADR-0002 cross-source tolerance.

Not validated:
- DATA-02-MBO book state and provider-internal replay evidence.
- Full L2/L3 feature parity.
- MBP10 size/order-count parity as hard gate evidence.

Decision:
DATA-01 remains blocked as a full gate. `DATA-01B` may proceed only for the MBP10
price-state sub-scope and the provider-internal MBO sub-scope; full `DATA-01B` remains
partially blocked pending implementation and replay evidence.
...
Full DATA-01 closure still requires DATA-02-MBO book state, DATA-03/DATA-04 evidence,
SIM-02/SIM-03 calibration, REL evidence, and a passing revised INFRA-01 verification report
that explicitly routes to `DATA-01`.
