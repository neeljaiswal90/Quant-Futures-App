...
export const VWAP_OVERNIGHT_REVERSAL_LONG_DEFAULTS = DEFAULT_VWAP_OVERNIGHT_REVERSAL_LONG_CONFIG;

export function generateVwapOvernightReversalLong(
  input: StrategyEvaluationInput,
): StrategyGenerationResult {
  if (input.strategy_id !== 'vwap_overnight_reversal_long') {
    throw new Error(`vwap_overnight_reversal_long generator received ${input.strategy_id}`);
  }

  const { snapshot } = input;
  const parameters = getStrategyParameters(input.strategy_config, 'vwap_overnight_reversal_long');
  const reasons: string[] = [];
  const rejection = firstVwapOvernightReversalLongRejection(snapshot, parameters, reasons);
  ...
  if (atr14 === null) {
    return {
      evaluation: makeEvaluation(snapshot, 'blocked', undefined, [
        'vwap_overnight_reversal_long:atr_unavailable',
        ...reasons,
      ]),
    };
  }
  ...
  if (!(riskPts > 0)) {
    return {
      evaluation: makeEvaluation(snapshot, 'blocked', undefined, [
        'vwap_overnight_reversal_long:non_positive_risk',
        ...reasons,
      ]),
    };
  }
  ...
  if (targets === null) {
    return {
      evaluation: makeEvaluation(snapshot, 'blocked', undefined, [
        'vwap_overnight_reversal_long:vwap_target_unavailable',
        ...reasons,
      ]),
    };
  }
  ...
  const candidate: Candidate = {
    candidate_id: makeCandidateId(`candidate-${snapshot.feature_snapshot_id}-vwap_overnight_reversal_long`),
    strategy_id: 'vwap_overnight_reversal_long',
    setup_type: 'vwap_overnight_reversal_long',
    ...
    reward_risk: targets.map((target) => ({
      label: target.label,
      reward_risk: rewardRiskVwapOvernight(target.price, entryPrice, riskPts, 'long'),
    })),
    confidence,
    config: snapshot.config,
    reasons: [
      'vwap_overnight_reversal_long:armed',
      `vwap_overnight_reversal_long:regime_${snapshot.context.regime_label}`,
      'vwap_overnight_reversal_long:target_1_vwap_touch',
      ...reasons,
    ],
  };
...
export function firstVwapOvernightReversalLongRejection(
  ...
  if (parameterIssues[0] !== undefined) {
    return `vwap_overnight_reversal_long:${parameterIssues[0]}`;
  }
  if (!snapshot.session.is_rth) {
    return 'vwap_overnight_reversal_long:session_not_rth';
  }
  if (snapshot.session.is_halt) {
    return 'vwap_overnight_reversal_long:session_halted';
  }
  if (snapshot.session.is_roll_block) {
    return 'vwap_overnight_reversal_long:roll_block_active';
  }

  const regime = snapshot.context.regime_label;
  if (regime === 'unknown') {
    return 'vwap_overnight_reversal_long:missing_regime_label';
  }
  if (!isVwapOvernightTradingRegime(regime)) {
    return 'vwap_overnight_reversal_long:regime_state_non_trading';
  }

  if (snapshot.context.opening_range_minutes_elapsed < parameters.exclude_first_minutes) {
    return 'vwap_overnight_reversal_long:warmup_period_not_complete';
  }

  const overnightBps = snapshot.context.overnight_return_bps;
  if (overnightBps === null) {
    return 'vwap_overnight_reversal_long:missing_overnight_data';
  }
  ...
  if (overnightBps > -parameters.min_abs_overnight_return_bps) {
    return 'vwap_overnight_reversal_long:overnight_magnitude_below_threshold';
  }

  const signedShock = selectVwapSignedShockValue(snapshot);
  if (signedShock === null) {
    return 'vwap_overnight_reversal_long:signed_shock_unavailable_warmup';
  }
  reasons.push(`signed_shock_vwap:${round4VwapOvernight(signedShock)}`);
  if (signedShock > -thresholdForRegime(regime, parameters)) {
    return 'vwap_overnight_reversal_long:signed_shock_below_threshold';
  }

  const adx = snapshot.indicators.adx_14;
  if (typeof adx === 'number' && Number.isFinite(adx) && adx > parameters.adx_max) {
    return 'vwap_overnight_reversal_long:adx_indicates_trend_environment';
  }
...
function makeEvaluation(
  ...
  return {
    strategy_evaluation_id: makeStrategyEvaluationId(
      `eval-${snapshot.feature_snapshot_id}-vwap_overnight_reversal_long`,
    ),
    strategy_id: 'vwap_overnight_reversal_long',
    instrument: snapshot.instrument,
    feature_snapshot_id: snapshot.feature_snapshot_id,
