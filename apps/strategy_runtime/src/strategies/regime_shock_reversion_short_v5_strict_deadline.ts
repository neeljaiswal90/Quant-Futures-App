import {
  DEFAULT_REGIME_SHOCK_REVERSION_SHORT_V5_STRICT_DEADLINE_CONFIG,
...
export const REGIME_SHOCK_REVERSION_SHORT_V5_STRICT_DEADLINE_DEFAULTS =
  DEFAULT_REGIME_SHOCK_REVERSION_SHORT_V5_STRICT_DEADLINE_CONFIG;

export function generateRegimeShockReversionShortV5StrictDeadline(
  input: StrategyEvaluationInput,
): StrategyGenerationResult {
  if (input.strategy_id !== 'regime_shock_reversion_short_v5_strict_deadline') {
    throw new Error(`regime_shock_reversion_short_v5_strict_deadline generator received ${input.strategy_id}`);
  }

  const { snapshot } = input;
  const parameters = getStrategyParameters(input.strategy_config, 'regime_shock_reversion_short_v5_strict_deadline');
  const reasons: string[] = [];
  const rejection = firstRegimeShockReversionShortV5StrictDeadlineRejection(snapshot, parameters, reasons);
  ...
  if (!(riskPts > 0)) {
    return {
      evaluation: makeEvaluation(snapshot, 'blocked', undefined, [
        'regime_shock_reversion_short_v5_strict_deadline:non_positive_risk',
        ...reasons,
      ]),
    };
  }
  ...
  if (
    ...
    return {
      evaluation: makeEvaluation(snapshot, 'blocked', undefined, [
        'regime_shock_reversion_short_v5_strict_deadline:targets_invalid',
        ...reasons,
      ]),
    };
  }
  ...
  const candidate: Candidate = {
    candidate_id: makeCandidateId(`candidate-${snapshot.feature_snapshot_id}-regime_shock_reversion_short_v5_strict_deadline`),
    strategy_id: 'regime_shock_reversion_short_v5_strict_deadline',
    setup_type: 'regime_shock_reversion_short_v5_strict_deadline',
    ...
    reasons: [
      'regime_shock_reversion_short_v5_strict_deadline:armed',
      `regime_shock_reversion_short_v5_strict_deadline:regime_${snapshot.context.regime_label}`,
      ...reasons,
    ],
  };
...
export function firstRegimeShockReversionShortV5StrictDeadlineRejection(
  ...
  if (parameterIssues[0] !== undefined) {
    return `regime_shock_reversion_short_v5_strict_deadline:${parameterIssues[0]}`;
  }
  if (!snapshot.session.is_rth) {
    return 'regime_shock_reversion_short_v5_strict_deadline:session_not_rth';
  }
  if (snapshot.session.is_halt) {
    return 'regime_shock_reversion_short_v5_strict_deadline:session_halted';
  }
  if (snapshot.session.is_roll_block) {
    return 'regime_shock_reversion_short_v5_strict_deadline:roll_block_active';
  }

  const regime = snapshot.context.regime_label;
  if (regime === 'unknown') {
    return 'regime_shock_reversion_short_v5_strict_deadline:missing_regime_label';
  }
  if (!isTradingRegime(regime)) {
    return 'regime_shock_reversion_short_v5_strict_deadline:regime_state_non_trading';
  }

  const shock = snapshot.context.signed_shock_vwap.value;
  if (shock === null) {
    return 'regime_shock_reversion_short_v5_strict_deadline:signed_shock_unavailable_warmup';
  }
  reasons.push(`signed_shock_vwap:${round4(shock)}`);

  if (regime === 'high' && shock < parameters.high_shock_threshold_pos) {
    return 'regime_shock_reversion_short_v5_strict_deadline:high_regime_shock_below_pos_threshold';
  }
  if (regime === 'low' && shock < parameters.low_shock_threshold_pos) {
    return 'regime_shock_reversion_short_v5_strict_deadline:low_regime_shock_below_strict_pos_threshold';
  }
...
function makeEvaluation(
  ...
  return {
    strategy_evaluation_id: makeStrategyEvaluationId(
      `eval-${snapshot.feature_snapshot_id}-regime_shock_reversion_short_v5_strict_deadline`,
    ),
    strategy_id: 'regime_shock_reversion_short_v5_strict_deadline',
    instrument: snapshot.instrument,
    feature_snapshot_id: snapshot.feature_snapshot_id,
...
function getRequiredNumber(values: StrategyScalarMap, key: string): number {
  ...
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    throw new Error(`regime_shock_reversion_short_v5_strict_deadline requires numeric ${key}`);
  }
  return value;
}
