...
export type FeatureFieldId =
  ...
  | 'mbp10_bid_levels_px'
  | 'mbp10_ask_levels_px'
  | 'mbp10_price_ladder_summary'
  | 'mbp10_price_state_freshness'
  | 'l1_mbp10_top_of_book_consistency'
  | 'mbo_lifecycle_event'
  | 'mbo_action'
  | 'mbo_side'
  | 'mbo_price'
  ...
  | 'mbo_spread_ticks'
  | 'mbo_mid_px'
  | 'mbo_active_order_count'
  | 'mbo_bid_level_count'
  | 'mbo_ask_level_count'
  | 'mbo_level_aggregate_size'
  | 'mbo_level_order_count'
  | 'queue_position_estimate'
  | 'queue_ahead_size_estimate'
  | 'queue_ahead_order_count_estimate'
  | 'microstructure_spread_points'
  | 'microstructure_spread_ticks'
  | 'microstructure_mid_px'
  | 'mbo_top_of_book_size_imbalance'
  | 'mbo_microprice_offset_ticks'
  | 'mbo_ofi_short'
  | 'mbo_ofi_medium'
  | 'mbo_ofi_blend'
  | 'trade_aggressor_imbalance'
  | 'mbo_recent_depth_imbalance'
  | 'mbo_queue_imbalance'
  | 'queue_ahead_fraction_estimate'
  | 'mbp10_size_diagnostic'
  | 'mbp10_order_count_diagnostic'
  | 'mbp10_size_summary_diagnostic'
  | 'mbp10_order_count_summary_diagnostic'
  | 'microprice_size_weighted'
  | 'top_of_book_size_imbalance'
...
export interface FeatureAvailabilityMask {
  readonly schema_version: typeof FEATURE_AVAILABILITY_MASK_SCHEMA_VERSION;
  readonly mask_version: typeof FEATURE_AVAILABILITY_MASK_VERSION;
...
} as const satisfies Readonly<Record<FeatureFieldId, string>>;

export function buildFeatureAvailabilityMask(): FeatureAvailabilityMask {
  const core = {
  ...
  return {
    ...core,
    mask_hash: hashMaskCore(core),
  };
}

export const FEATURE_AVAILABILITY_MASK = buildFeatureAvailabilityMask();

export function tierOf(
  mask: FeatureAvailabilityMask,
  field: FeatureFieldId,
): FeatureAvailabilityTier {
  return mask.field_tiers[field];
}

export function assertAuthoritative(mask: FeatureAvailabilityMask, field: FeatureFieldId): void {
  const tier = tierOf(mask, field);
...
function hashMaskCore(core: Omit<FeatureAvailabilityMask, 'mask_hash'>): string {
  return `sha256:${createHash('sha256').update(stableJsonStringify(core as JsonValue)).digest('hex')}`;
}
