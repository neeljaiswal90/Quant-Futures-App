...
import {
  generateRegimeShockReversionShortV5TrailAtDeadline,
...
describe('regime_shock_reversion_short_v5_trail_at_deadline gate stack', () => {
  it('arms on high-regime positive VWAP shock above the tightened v2 threshold', () => {
    const result = generateRegimeShockReversionShortV5TrailAtDeadline({
  ...
  it('blocks high-regime shocks below the v2 threshold', () => {
    ...
    expect(result.evaluation.reasons[0]).toBe(
      'regime_shock_reversion_short_v5_trail_at_deadline:high_regime_shock_below_pos_threshold',
    );
  });
  ...
  it('blocks low-regime shocks below the stricter low-regime threshold', () => {
    ...
    expect(result.candidate).toBeUndefined();
    expect(result.evaluation.reasons[0]).toBe(
      'regime_shock_reversion_short_v5_trail_at_deadline:low_regime_shock_below_strict_pos_threshold',
    );
  });
  ...
  it('fails closed when the signed-shock feature is missing', () => {
    ...
    expect(() => generateRegimeShockReversionShortV5TrailAtDeadline({
      strategy_id: 'regime_shock_reversion_short_v5_trail_at_deadline',
  ...
  it('fails closed when the regime label is unavailable', () => {
    ...
    expect(result.candidate).toBeUndefined();
    expect(result.evaluation.reasons[0]).toBe(
      'regime_shock_reversion_short_v5_trail_at_deadline:missing_regime_label',
    );
  });
});
...
function withRegimeAndShock(
  ...
  const base = STRATEGY_SYNTHETIC_FIXTURES.regime_shock_reversion_short_v5_trail_at_deadline.snapshot;
  const anchor = 18600;
  const sigma = 6;
...
describe('regime_shock_reversion_short_v5_trail_at_deadline registration', () => {
  const strategyId = 'regime_shock_reversion_short_v5_trail_at_deadline';
  ...
  it('resolves the expected ADR-0027 management profile', () => {
    ...
    expect(profile.time_stop.at_deadline_extension).toBe('activate_trail');
    ...
    expect(profile.trailing_stop.enabled).toBe(true);
    expect(profile.trailing_stop.distance_ticks).toBe(8);
    expect(profile.trailing_stop.mode).toBe('post_pt1_ticks');
  });
  ...
  it('rejects activate_trail profile mutations with disabled trailing stops', () => {
    const resolvedProfile = resolveManagementProfileForV5Registration(strategyId);
    const profile = resolvedProfile.profile;
    const issues = validateManagementProfileForV5Registration({
      ...profile,
      trailing_stop: {
    ...
    expect(issues.some((issue) => issue.message.includes('activate_trail') && issue.message.includes('trailing_stop.enabled'))).toBe(true);
  });

  it('rejects activate_trail profile mutations with disabled trailing-stop mode', () => {
    const resolvedProfile = resolveManagementProfileForV5Registration(strategyId);
    const profile = resolvedProfile.profile;
    const issues = validateManagementProfileForV5Registration({
      ...profile,
    ...
    expect(issues.some((issue) => issue.message.includes('activate_trail') && issue.message.includes('post_pt1_ticks'))).toBe(true);
  });
});