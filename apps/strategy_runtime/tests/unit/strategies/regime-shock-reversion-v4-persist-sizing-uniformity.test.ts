...
describe('regime_shock_reversion_short_v4_persist: sizing uniformity across persistence arming buckets', () => {
  ...
  for (const regime of regimes) {
    for (const shock of signedShockCells) {
      it(`keeps risk sizing stable when candidate fires (regime=${regime}, shock=${shock})`, () => {
        const baseline = generateRegimeShockReversionShortV4Persist({
...
function withRegimeShockAndRecent(
  ...
  return {
    ...
    context: {
      ...
      signed_shock_vwap_recent_values: [shock, shock, shock],
    },
  };
}