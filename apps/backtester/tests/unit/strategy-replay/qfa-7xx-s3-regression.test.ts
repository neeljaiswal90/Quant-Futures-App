...
const S3_STRATEGY_IDS = [
  'regime_mean_reversion_long',
  'regime_mean_reversion_short',
] as const;

describe('QFA-7xx-S3 replay regression baselines', () => {
  it('locks initial Strategy 3 replay behavior for future fingerprint bumps', async () => {
    const result = await replayStrategies({
      strategy_ids: S3_STRATEGY_IDS,
      bars: REPLAY_BARS,
    });

    for (const strategyId of S3_STRATEGY_IDS) {
