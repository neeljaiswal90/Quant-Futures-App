...
const S1_STRATEGY_IDS = [
  'vwap_overnight_reversal_long',
  'vwap_overnight_reversal_short',
] as const;

describe('QFA-7xx-S1 replay regression baselines', () => {
  it('locks the VWAP overnight reversal strategies against QFA-301 replay drift', async () => {
    const result = await replayStrategies({
      strategy_ids: S1_STRATEGY_IDS,
      bars: REPLAY_BARS,
    });
    const evaluations = result.evaluations as readonly ReplayEvaluationBehavior[];

    for (const strategyId of S1_STRATEGY_IDS) {
